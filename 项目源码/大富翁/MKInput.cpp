...
void MKInput::Initialize(HWND hWnd)
{
	if (hWnd && !m_hWnd)
	{
		m_hWnd = hWnd;
...
void MKInput::UpdateMouseButtonStatus()
{
	for (int i = 0; i < 3; ++i)
	{
		// 这一帧按键按下
		...
		{
			if (m_MouseStatus[i] == MouseClick::MOUSE_CLICK_DEFAULT ||
				m_MouseStatus[i] == MouseClick::MOUSE_CLICK_UP)
				m_MouseStatus[i] = MouseClick::MOUSE_CLICK_DOWN;
			else
				m_MouseStatus[i] = MouseClick::MOUSE_CLICK_KEEP;
		}
		// 这一帧没按下
		else
		{
			if (m_MouseStatus[i] == MouseClick::MOUSE_CLICK_DEFAULT ||
				m_MouseStatus[i] == MouseClick::MOUSE_CLICK_UP)
				m_MouseStatus[i] = MouseClick::MOUSE_CLICK_DEFAULT;
			else
				m_MouseStatus[i] = MouseClick::MOUSE_CLICK_UP;
		}
	}
